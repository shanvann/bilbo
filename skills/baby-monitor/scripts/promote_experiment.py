...
from lib.config import MODELS_DIR, DATA_DIR  # noqa: E402


MANIFEST = SKILL_DIR / "scripts" / "lib" / "experiments.json"
...
def preflight(args) -> dict:
    ...
    try:
        from lib.training_state import is_running as _train_is_running
...
def backfill_new_shadow(ctx: dict, dry_run: bool):
    if dry_run:
        ...
        return
    cmd = [
        sys.executable, str(SKILL_DIR / "scripts" / "experiments_backfill.py"),
        "--name", ctx["legacy_name"],
        "--force",
    ]
...
def reinfer_prod_on_history(dry_run: bool):
    ...
    from lib.local_pipeline import (  # noqa: PLC0415
