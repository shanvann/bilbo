...
```bash
curl -s http://192.168.1.50/measures/current | python -m json.tool
```
...
| ----------------- | -------------------------------------- | --------------------------- |
| `AIRGRADIENT_URL` | `http://192.168.1.50/measures/current` | Full URL including path     |
...
1. **DHCP reservation (preferred for headless setups).** Reserve the
   AirGradient's MAC to a fixed IP in your router (e.g. `192.168.1.50`). Robust
