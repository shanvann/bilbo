...
| `state` | Asleep, Awake, FallingAsleep, Unknown, not_present | **Temporally smoothed** (2026-04-14). Only flips to Awake/Asleep after 4 consecutive agreeing `eyeState` readings (`eyes_open` / `eyes_closed`) in the last 6 baby-present frames. Between flips, the previous smoothed state is carried forward. `FallingAsleep` (added 2026-04-20) is produced only by the putdown-pattern absorption (`not_present → Unknown+babyPresent ≤30 min → Asleep`); see `FALLING_ASLEEP_MAX_MINUTES` in `lib/config.py`. See `lib/state.py` + `STATE_CONFIRM_WINDOW` / `STATE_CONFIRM_RUN`. |
...
**Putdown-pattern absorption / FallingAsleep (2026-04-20).** Companion to Unknown→Awake but targets the mirror case: `not_present → Unknown+babyPresent → Asleep`. When the smoother confirms `Asleep` AND the preceding contiguous `Unknown+babyPresent` run is bookended by `not_present`, the run is reclassified by span: `≤ FALLING_ASLEEP_MAX_MINUTES` (default 30) → `FallingAsleep`; `> 30 min` → `Awake` (crib-awake before dozing off). The narrow `not_present` precondition is deliberate — a generic `Awake → Unknown → Asleep` pattern (no removal) is not absorbed, because pre-sleep ambiguity is a weaker signal than the sleep confirmation itself (same asymmetric rationale as above). `FallingAsleep` is a first-class state value rendered distinctly in the dashboard (light green, between Awake-yellow and Asleep-green). Alerts are unaffected: `should_alert_asleep` requires a prior `Awake` in the lookback, which `FallingAsleep` doesn't satisfy — so the putdown case stays silent. Helper: `lib.state.putdown_prefix_to_absorb`. `scripts/backfill_state.py` applies it as pass-3 after Unknown→Awake (pass 2 wins when a run is eligible for both, by design).
